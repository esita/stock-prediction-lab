...
def plotly_table(dataframe):
    headerColor = 'grey'
    rowEvenColor = '#f8fafd'
    rowOddColor = '#e1efff' # Corrected from #elefff to #e1efff
    ...
    fig = go.Figure(data=[go.Table(
        ...
        cells=dict(
            ...
            # This ensures every row alternates colors correctly
...
def filter_data(dataframe, num_period):
    if num_period == '1mo':
        ...
        date = dataframe.index[-1] + dateutil.relativedelta.relativedelta(months=-5)
    elif num_period == '6mo':
...
def candlestick(dataframe, num_period):
     ...
     fig.update_layout(showlegend = False, height=500, margin=dict(l=0, r=20, t=20, b=0), plot_bgcolor = 'white', paper_bgcolor='#E1E1FF')
     return fig
def RSI(dataframe, num_period):
    ...
    height=200,plot_bgcolor='white' , paper_bgcolor='#E1E1FF',margin=dict(l=0, r=0, t=0, b=0),legend=dict(orientation="h",            
    yanchor="top",
    y=1.02,
    xanchor="right",
    x=1))
...
def Moving_average(dataframe, num_period):
    ...
    fig.add_trace(go.Scatter(x=dataframe['Date'], y=dataframe['Open'], mode='lines', name='Open', line=dict(width=2, color='#5ab7ff')))
    fig.add_trace(go.Scatter(x=dataframe['Date'], y=dataframe['Close'], mode='lines', name='Close', line=dict(width=2, color='black')))
    fig.add_trace(go.Scatter(x=dataframe['Date'], y=dataframe['High'], mode='lines', name='High', line=dict(width=2, color='#E1E1FF')))
    fig.add_trace(go.Scatter(x=dataframe['Date'], y=dataframe['Low'], mode='lines', name='Low', line=dict(width=2, color='red')))
